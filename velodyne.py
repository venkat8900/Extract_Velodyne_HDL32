...
def read_firing_data(data, idx, fd_callback):
    ...
    # EVAN: Removed this next line for debugging
    # print("block_id:",block_id, "idx:", idx)
    # assert block_id == 0xeeff or block_id == 0xddff
    if block_id == 0xeeff: # or block_id == 0xddff:
        print("assert fail.. continuing", block_id)

    idx += 2
    rot_pos = read_uint16(data, idx) / 100
    # print("Rotational position:",rot_pos)
    idx += 2

    for l in range(32):
        # print("l:",l) EVAN DEBUG
        dist = read_uint16(data, idx)
        idx += 2
        # print(l, idx, dist) EVAN DEBUG
        # print(l, idx, "inten",read_uint8(data, idx))
        ...
        # print("***** laser_idx****",laser_idx) EVAN DEBUG
...
def process_frame(data, idx, status, fd_callback):
    # print("1231243252154235234523452345324532452 -- data length:", len(data)) EVAN DEBUG
    for b in range(12):
        # print("Frame:",b)
...
def calc_coords(dist, rot, cal):
    ...
    if rot == 360.0:
        print("Skipped because of 360.0 degrees.. Produces error, come back to this")
    else:
